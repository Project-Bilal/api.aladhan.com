...
class PrayerTimes
{
    ...
    public ?string $customGoogleApiKey;
    public string $GoogleGeocodeBaseUrl;
    public string $GoogleTimezoneBaseUrl;
    ...
    public function __construct(ContainerInterface $container, ServerRequestInterface $request, MemcachedAdapter $mc)
    {
        ...
        $this->mc = $mc;
        $this->customGoogleApiKey = Request::getQueryParam($request, 'x7xapikey');
        if ($this->customGoogleApiKey === null) { // This means no custom Google API key was passed. Use default.
            $this->GoogleApiKey = $c['apikey'];
        } else {
            // Use the custom Google API key passed by the user.
            $this->GoogleApiKey = $this->customGoogleApiKey;
        }
        $this->GoogleGeocodeBaseUrl = $c['geocode_baseurl'];
        $this->GoogleTimezoneBaseUrl = $c['timezone_baseurl'];
        ...
        $this->timezone = $this->mc->get(md5('tz.' . $this->latitude . '.' . $this->longitude . '.' . $timezoneString), function (ItemInterface $item) use ($request, $timezoneString) {
            ...
            return Timezone::computeTimezone($this->latitude, $this->longitude,
                $timezoneString, $this->GoogleApiKey, $this->GoogleTimezoneBaseUrl, $this->request);
        });
    ...
    public function validateAddress(): void
    {
        ...
        if ($this->address !== null && ApiRequest::isValidAddress($this->address)) {
            // /timingsByAddress call. Geocode using Google API.
            $coordinates = $this->mc->get(md5('addr.' . strtolower($this->address) . $this->GoogleApiKey), function (ItemInterface $item)  {
                $item->expiresAfter(7200);
                
                // Build Google Geocoding API URL
                $address = urlencode($this->address);
                $url = "https://maps.googleapis.com/maps/api/geocode/json?address={$address}&key={$this->GoogleApiKey}";
